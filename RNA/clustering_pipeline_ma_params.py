...
pg.highly_variable_features(data, consider_batch=True, n_top=2500)
pg.pca(data, n_components=40)
pca_key = pg.run_harmony(data, n_jobs=15)
pg.neighbors(data, rep=pca_key, K=25, n_jobs=15)
...
hodge_dict = pg.infer_cell_types(data,markers=hodge_markers,output_file=folder+'Hodge_infer_cell_types.txt')
...
plt.savefig(folder+'hodge_anno.png',bbox_inches='tight')

lake_dict = pg.infer_cell_types(data,markers=lake_markers,output_file=folder+'Lake_infer_cell_types.txt')
...
plt.savefig(folder+'lake_anno.png',bbox_inches='tight')

nenad_dict = pg.infer_cell_types(data,markers=nenad_markers,output_file=folder+'Nenad_infer_cell_types.txt')
...
plt.savefig(folder+'nenad_anno.png',bbox_inches='tight')

print('----------SAVE OUTPUT FILE----------')
pg.write_output(data, output)

print('----------GENE EXPRESSION PLOTS----------')
pg.scatter(data, attrs=['hodge_anno','n_genes'], wspace = 0.1, panel_size=(3,3),legend_loc=['on data','right margin'],basis='umap',dpi=300)
plt.savefig(folder+'ngenes.png',bbox_inches='tight')
pg.dotplot(data, genes=['SLC17A7','GAD1','FLT1','AQP4','MOBP','PDGFRA','APBB1IP'], groupby='hodge_anno')
plt.savefig(folder+'simple_dotplot.png', bbox_inches='tight')
