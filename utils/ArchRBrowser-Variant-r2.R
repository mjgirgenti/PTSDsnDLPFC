...
#' @param logFile The path to a file to be used for logging ArchR output.
#' @export
plotBrowserTrackVariant <- function(
  ArchRProj = NULL, 
  region = NULL, 
  groupBy = "Clusters",
  useGroups = NULL, 
  plotSummary = c("bulkTrack", "featureTrack", "loopTrack", "variantTrack1", "variantTrack2", "geneTrack"),
  sizes = NULL,
  features = getPeakSet(ArchRProj),
  variants1 = NULL,
  pip_pos = NULL,
  ...
  ggList <- lapply(seq_along(region), function(x){
    ...
    if("bulktrack" %in% tolower(plotSummary)){
      ...
      plotList$bulktrack <- .bulkTracks(
        ...
        useGroups = useGroups,
        tstart = tstart,
        logFile = logFile) + theme(plot.margin = unit(c(0, 0.75, -0.1, 0.75), "cm"))
    }
    ...
    if("featuretrack" %in% tolower(plotSummary)){
      if(!is.null(features)){
        ...
        plotList$featuretrack <- .featureTracks(
            ...
            hideX = TRUE, 
            title = "Peaks",
            logFile = logFile) + theme(plot.margin = unit(c(0, 0.75, 0, 0.75), "cm"))
      }
    }
    ...
    # Variant Tracks 1
    ##########################################################
    if("varianttrack1" %in% tolower(plotSummary)){
      if(variants1==TRUE){
        .logDiffTime(sprintf("Adding Variant Tracks (%s of %s)",x,length(region)), t1=tstart, verbose=verbose, logFile=logFile)
        plotList$varianttrack1 <- .variantTracks1(
            variants1 = variants1,
            pip_pos = pip_pos,
            oldsnp = oldsnp,
            gwas = gwas,
    ...
    # Variant Tracks 2
    ##########################################################
    if("varianttrack2" %in% tolower(plotSummary)){
      if(!is.null(variants2)){
        .logDiffTime(sprintf("Adding Variant Tracks (%s of %s)",x,length(region)), t1=tstart, verbose=verbose, logFile=logFile)
        plotList$varianttrack2 <- .variantTracks2(
            variants2 = variants2, 
            region = region[x], 
            facetbaseSize = facetbaseSize,
            hideX = TRUE, 
...
.bulkTracks <- function(
  ...
  title <- paste0(as.character(seqnames(region)),":", start(region), "-", end(region), " ", title)
  ...
  p <- ggplot(df, aes_string("x","y", color = "group", fill = "group")) + 
    ...
    scale_y_continuous(limits = ylim2, expand = c(0,0)) +
  ...
  if(nrow(genesO) > 0){
    ...
    if(length(which(genesO$strand!="-")) > 0){
      p <- p + ggrepel::geom_text_repel(data=genesO[which(genesO$strand!="-"),], 
        aes(x = start, y = cluster, label = symbol, color = strand, label.size=NA), 
          nudge_x = -0.001*(end(region) - start(region)), nudge_y = 0.5, 
    ...
    if(length(which(genesO$strand=="-")) > 0){
      p <- p + ggrepel::geom_text_repel(data=genesO[which(genesO$strand=="-"),], 
        aes(x = end, y = cluster, label = symbol, color = strand, label.size=NA), 
          nudge_x = +0.001*(end(region) - start(region)), nudge_y = 0.5, 
          size = labelSize, direction = "x", inherit.aes=FALSE)
    }
...
.variantTracks1 <- function(
  ...
  dataset <- read.table(glue('/home/ah2428/palmer_scratch/finemap_final/{gene}.{gwas}.{celltype}.0.1.result.txt'),header=T)  
  oldsnppos <- dataset[dataset$SNP_ID==oldsnp,]$start
  r1 <- dataset$SuSiE_PIP_O
  r2 <- dataset$SuSiE_PIP_A
  startposition <- dataset$start # CHANGE HERE
  snp <- dataset$SNP_ID
  whichid <- which(dataset$SNP_ID==snp)

  datapip <- data.frame(pip = c(r1,r2), snp = snp, position = rep(startposition,2), method = c(rep("original",length(r1)),rep("atacprior",length(r1))))
  ...
  if (gene=='KCNIP4') {
      ...
      datapip_atacprior$sig[datapip_atacprior$pos %in% pip_pos] = "Within peak"
      datapip_atacprior$sig[datapip_atacprior$pos %ni% pip_pos] = "Not within peak"
      #datapip_atacprior$sig[datapip_atacprior$pos %in% oldsnppos] = "MVP"
      datapip_atacprior <- datapip_atacprior[datapip_atacprior$pip > 0.0001,]
        
      r2_df <- read.table(glue('/home/ah2428/palmer_scratch/finemap_final/{gene}.{gwas}.{celltype}.0.1.r2.txt',sep='\t'))
      
      cre_snps <- subset(datapip_atacprior,sig=='Within peak')
      max_snp <- cre_snps[which.max(cre_snps$pip),]$snp
      #mvp_snp <- datapip_atacprior[datapip_atacprior$sig=='MVP',]$snp
      corrs <- c()
      for (snp in datapip_atacprior$snp) {
        c <- r2_df[max_snp,snp]
        corrs <- append(corrs,c)
      }
      datapip_atacprior$r2 <- corrs
      first_two <- subset(datapip_atacprior,sig=='Within peak')
      first_two <- first_two[order(-first_two$pip),][1:2,]
      old <- datapip_atacprior[datapip_atacprior$snp==oldsnp,]
      text_df <- rbind(first_two,old)
      
      p <- ggplot(datapip_atacprior, aes(x = position, y = pip),shape=21,size=3) + geom_point(aes(color=r2),size=3) + scale_color_gradient(low='skyblue',high='navyblue') + 
      geom_point(data=subset(datapip_atacprior,snp==max_snp), shape=21, size=3, fill='red', stroke=NA) +
      geom_point(data = subset(datapip_atacprior,sig=='Within peak'), color = "black", stroke = 0.5,shape = 21,size=3) +
    
      ylab("PIP") +
      scale_y_continuous(limits = c(0, max(datapip_atacprior$pip)+0.1), expand = c(0, 0)) +
      scale_x_continuous(limits = c(chromstart, chromend), expand = c(0, 0)) +
      ...
      theme(panel.grid.major = element_blank(), 
        ...
        element_line(size=0.5)) + geom_vline(xintercept = tad1) + geom_vline(xintercept = tad2) + 
    
      geom_text(data=text_df,aes(x=position, y=pip, label=snp),size=4) 
  } else {
      datapip_atacprior$sig[datapip_atacprior$pos %in% pip_pos] = "Within peak"
      datapip_atacprior$sig[datapip_atacprior$pos %ni% pip_pos] = "Not within peak"
      datapip_atacprior$sig[datapip_atacprior$pos %in% oldsnppos] = "MVP"
      ...
      p <- ggplot(datapip_atacprior, aes(x = position, y = pip),shape=21,size=3) + geom_point(aes(color=r2),size=3) + scale_color_gradient(low='skyblue',high='navyblue') + 
      geom_point(data=subset(datapip_atacprior,sig=='MVP'), fill='purple', color='purple', size=3, shape=21)+
      geom_point(data=subset(datapip_atacprior,snp==max_snp), shape=21, size=3, fill='red', stroke=NA) +
      geom_point(data = subset(datapip_atacprior,sig=='Within peak'), color = "black", stroke = 0.5,shape = 21,size=3) +
...
.variantTracks2 <- function(
  ...
  if(!is.null(variants2)){
    ...
    if(!.isGRList(variants2)){
      variants2 <- .validGRanges(variants2)
      featureList <- SimpleList(FeatureTrack = variants2)
      hideY <- TRUE
    }else{
      featureList <- variants2
      hideY <- FALSE
    }
    ...
    #create empty plot
    df <- data.frame(facet = "VariantTrack", start = 0, end = 0, strand = "*", symbol = "none")
    p <- ggplot(data = df, aes(start, end)) + 
      geom_point() +
      facet_grid(facet~.) +
      theme_ArchR(baseSize = baseSize, baseLineSize = borderWidth, baseRectSize = borderWidth) +
      ...
      theme(axis.title.x=element_blank(), axis.text.x=element_blank(),axis.ticks.x=element_blank()) +
      theme(axis.title.y=element_blank(), axis.text.y=element_blank(),axis.ticks.y=element_blank()) +
      theme(legend.position = c(0.95, 0.5),legend.title = element_blank())
  ...
  if(!is.ggplot(p)){
    .logError("variantTrack is not a ggplot!", fn = ".variantTracks", info = "", errorList = NULL, logFile = logFile)
  }
...
.loopTracks <- function(
  ...
  if(!is.null(loops)){
    ...
    if(testDim){
      ...
      loopO$facet <- " " 
      if (is.null(pal)) {
        #pal <- colorRampPalette(c("#E6E7E8","#3A97FF","#8816A7","black"))(100) # default 
        if(celltype=='EXC') {
            pal <- colorRampPalette(c("#FCBBA1","#FC9272","#EF3B2C","#99000D"))(100) # red
        } else if (celltype=='INH') {
            pal <- colorRampPalette(c("#C7E9C0","#A1D99B","#41AB5D","#005A32"))(100) # green
        } else {
            pal <- colorRampPalette(c("#D9D9D9","#969696","#525252","black"))(100) # grey
        }
      }
      ...
      p <- ggplot(data = data.frame(loopO), aes(x = x, y = y, group = id, color = value)) + 
        ...
        #theme(strip.text.y = element_text(size = facetbaseSize, angle = 0), strip.background = element_blank(), legend.box.background = element_rect(color = NA)) +
        theme(strip.text.y = element_blank(), strip.background = element_blank(), legend.box.background = element_rect(color = NA)) +
        guides(color= guide_colorbar(barheight = 1, barwidth = 5)) +
        theme(legend.direction="horizontal", legend.position = c(0.8,0.55), legend.text=element_text(size=10), legend.margin=margin(0,0,0,0)) + labs(colour = "Corr") 
