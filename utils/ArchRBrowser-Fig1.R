...
#' @param logFile The path to a file to be used for logging ArchR output.
#' @export
plotBrowserTrack3 <- function(
  ArchRProj = NULL, 
  region = NULL, 
  groupBy = "Clusters",
  useGroups = NULL, 
  plotSummary = c("bulkTrack", "featureTrack", "variantTrack1", "variantTrack2", "loopTrack", "geneTrack"),
  sizes = NULL,
  features = getPeakSet(ArchRProj),
  variants1 = NULL,
  variants2 = NULL,
  ...
  ggList <- lapply(seq_along(region), function(x){
    ...
    if("bulktrack" %in% tolower(plotSummary)){
      ...
      plotList$bulktrack <- .bulkTracks(
        ...
        useGroups = useGroups,
        tstart = tstart,
        logFile = logFile) + theme(plot.margin = unit(c(0.1, 0.75, 0, 0.75), "cm"))
    }
...
.variantTracks1 <- function(
  ...
  if(!is.null(variants1)){
    ...
    if(!.isGRList(variants1)){
      variants1 <- .validGRanges(variants1)
      featureList <- SimpleList(FeatureTrack = variants1)
      hideY <- TRUE
    }else{
      featureList <- variants1
      hideY <- FALSE
    }
...
.loopTracks <- function(
  ...
  if(!is.null(loops)){
    ...
    if(testDim){
      ...
      loopO$facet <- title
      if(is.null(pal)){
        pal <- colorRampPalette(c("#D9D9D9","#969696","#525252","black"))(100)
        #pal <- colorRampPalette(c("#E6E7E8","#3A97FF","#8816A7","black"))(100)
        #pal <- colorRampPalette(c("#FEE0D2","#FC9272","#EF3B2C","#99000D"))(100)
        #pal <- colorRampPalette(c("#E5F5E0","#A1D99B","#41AB5D","#005A32"))(100)
      }
