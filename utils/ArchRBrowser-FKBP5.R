...
#' @param logFile The path to a file to be used for logging ArchR output.
#' @export
plotBrowserTrackFKBP5 <- function(
  ArchRProj = NULL, 
  region = NULL, 
  groupBy = "Clusters",
  useGroups = NULL, 
...
.variantTracks1 <- function(
  ...
  work_dir = "/gpfs/gibbs/pi/gerstein/jz435/ShareZhangLab/PTSD/variantFineMap/"

  dataset <- read.table('/gpfs/gibbs/pi/gerstein/jz435/ShareZhangLab/PTSD/variantFineMap/ELFN1INHresultsusienew.txt', header = T)

  phenotype <- 'totalPCL' # CHANGE HERE
  gene <- 'ELFN1' # CHANGE HERE
  cell_type <- 'INH' # CHANGE HERE

  chrom = "chr7" # CHANGE HERE
  chromstart = 1400000 # CHANGE HERE
  chromstartGeneSel = 1400000 # CHANGE HERE
  chromend = 2500000 # CHANGE HERE

  r1 <- dataset$SuSiE_PIP_O
  r2 <- dataset$SuSiE_PIP_A
  startposition <- dataset$start # CHANGE HERE
  snp <- dataset$SNP_ID
  peakindex <- which(dataset$peak_indicator>0.1)
  ...
  pip_pos <- c(1814997,1838740,1843637,1845300,1845963,2020125,2069863)
  datapip_atacprior = datapip[datapip$method == "atacprior",]
  datapoint_atacprior = datapoint[datapoint$method == "atacprior",]

  datapip_atacprior$sig[datapip_atacprior$pip < 0.05] = "Sig1"
  datapip_atacprior$sig[datapip_atacprior$pip >= 0.05 & datapip_atacprior$pip < 0.1] = "Sig1"
  datapip_atacprior$sig[datapip_atacprior$pip >= 0.1 & datapip_atacprior$pip < 0.2] = "Sig1"
  datapip_atacprior$sig[datapip_atacprior$pip >= 0.2 & datapip_atacprior$pip < 0.5] = "Sig1"
  datapip_atacprior$sig[datapip_atacprior$pip >= 0.5] = "Sig5"
  datapip_atacprior$sig[datapip_atacprior$pos %in% pip_pos] = "Sig5"

  sig.cols = c("Sig1" = "#000080", "Sig2" = "#68BBE3", "Sig3" = "#32CD30", "Sig4" = "#FD7F20", "Sig5" = "#FF0000")

  #create empty plot
  df <- data.frame(facet = "VariantTrack", start = 0, end = 0, strand = "*", symbol = "none")

  p <- ggplot(datapip_atacprior, aes(x = position, y = pip, color = sig, fill = sig)) +
  geom_point(size = 1, shape = 21) +
  ylab("PIP") +
  scale_y_continuous(limits = c(0, max(datapip_atacprior$pip)+0.05), expand = c(0, 0)) +
  scale_x_continuous(limits = c(chromstart, chromend), expand = c(0, 0)) +
  scale_fill_manual(values = sig.cols, drop = FALSE) +
  scale_color_manual(values = sig.cols, guide = "none", drop = FALSE) + 
  ...
  if(!is.null(loops)){
    ...
    if(testDim){
      ...
      if(is.null(pal)){
        ...
        #pal <- colorRampPalette(c("#FEE0D2","#FC9272","#EF3B2C","#99000D"))(100) # red
        #pal <- colorRampPalette(c("#E5F5E0","#A1D99B","#41AB5D","#005A32"))(100) # green
      }
      ...
      p <- ggplot(data = data.frame(loopO), aes(x = x, y = y, group = id, color = value)) + 
        ...
        guides(color= guide_colorbar(barheight = 0.5, barwidth = 3)) +
        theme(legend.direction="horizontal", legend.title = element_blank(), legend.position = c(0.9,0.5))
