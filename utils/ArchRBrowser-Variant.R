...
.variantTracks1 <- function(
  ...
  #dataset <- read.table(glue('/gpfs/gibbs/pi/gerstein/jz435/ShareZhangLab/PTSD/variantFineMap/xiangyu_param1/{gwas}/{gene}{celltype}resultsusienew.txt'), header = T)
  ...
  datapip_atacprior$sig[datapip_atacprior$pos %in% oldsnppos] = "Old SNP"
  datapip_atacprior <- datapip_atacprior[datapip_atacprior$pip > 0.001,]

  #sig.cols = c("Within peak" = "#FF0000","Not within peak" = "gray")
  sig.cols = c("Within peak" = "#FF0000","Not within peak" = "lightgray", "Old SNP" = "blue")
  
  first_two <- datapip_atacprior[order(-datapip_atacprior$pip),][1:2,]
  ...
  p <- ggplot(datapip_atacprior, aes(x = position, y = pip, fill = sig)) +
